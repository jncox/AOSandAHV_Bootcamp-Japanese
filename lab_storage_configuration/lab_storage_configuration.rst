...
HCI分野でのパイオニアとして、DSFはエンタープライズデータベース、仮想デスクトップ、ROBO、ビックデータなどの様々なワークロードをサポートするために必要なパフォーマンスと耐障害性を提供できる成熟したソリューションです。
...
#. 別のストレージコンテナーを選択して、画面下部の **Storage Container Details** を確認しましょう。
   ...
   **Overall Efficiency** は、データ削減だけなく、シン・プロビジョニングやクローニングのようなデータを節約する、DSFにネイティブのデータアボイダンスをトラックします。
...
Nutanix クラスタはブロックまたはラックのレベルで `可用性ドメイン <https://nutanixbible.com/#anchor-book-of-aos-availability-domains>`_の適用も可能です。

Block Awarenessは、十分に大規模なクラスタでは、データのセカンダリコピーがプライマリコピーと同じ物理的エンクロージャー内のノードに書き込まれないようにします。
これにより、マルチノードブロックのデータ損失を回避することができます。
同じコンセプトは、複数のラックにまたがるNutanixクラスタを使用して適用することができます。

ラック/ブロック・フォルトトレランスの基本的な要件は、メタデータの3つのコピーを保存する必要があるため、クラスタ内に最低3つのブロックがあることです（RF2の場合）。
AOS 5.8からは、イレイジャーコーディングを有効にしてRack AwarenessとBlock Awarenessをサポートすることができます。
...
- Storage Containersでは、RFレベル、圧縮、重複排除、イレイジャーコーディングなど、VMのストレージポリシーを定義することができます
